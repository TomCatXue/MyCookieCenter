...
## 📌 Loon 插件元信息规范 (版本与时分强制要求)

所有位于 `loon/` 目录下的 `.plugin` 文件，必须严格遵守以下元信息规范：

1. **强制携带版本与日期**：所有插件头部必须包含 `#!version` 与 `#!date` 字段；
2. **日期必须精确到时分**：`#!date` 格式必须为 **`YYYY-MM-DD HH:mm`**（例如 `2026-09-17 11:50`），严禁只写年月日；
3. **更新时必须同步修改**：任何对插件配置、脚本逻辑或依赖链接的修改推送，**必须同步递增 `#!version` 并将 `#!date` 更新为修改时的精准时分**，方便使用者清晰校验是否获取到最新版本、排查客户端缓存。
