...
最简单的方式：使用本仓库的 [`loon/CookieCenter.plugin`](../loon/CookieCenter.plugin)（微信读书 / 山西电信）；其余脚本按其 `README.md` 中的平台配置段设置，只需：

1. 安装 [`loon/CookieCenter.plugin`](../loon/CookieCenter.plugin)（微信读书 / 山西电信已内置 MITM 与 cron）；独立功能脚本按其 README 配置重写规则。
