...
"""
===================================================================
📌 版本: v1.0.1 (2026-09-20 纯净原版规范版)
中国电信 · 每日签到与金豆任务聚合脚本 (基于原版0716通道)
===================================================================
...
def login_v2(phone: str, password: str, android_id: str):
    ...
    if not login_data:
        err_msg = data_block.get('resultMsg') or resp_data.get('resultDesc') or res.get('headerInfos', {}).get('reason') or '服务密码有误或触发风控'
...
def sign_tasks(user: dict) -> list:
    ...
    for _ in range(10):
        ...
        if "最大" in msg or "已达" in msg or not msg:
            break
        feed_done += 1
        feed_status = f"成功投喂 {feed_done} 次"
        time.sleep(1)
...
def main():
    ...
    if not raw.strip():
        print("\n❌ 未找到环境变量 dxlin，请按格式设置：手机号#密码#AndroidID，多账号换行分隔\n")
        return

    accs = [re.split(r'[#@]', line.strip()) for line in re.split(r'[&\r\n]+', raw) if line.strip() and ('#' in line or '@' in line)]
    if not accs:
        print("\n❌ 未解析到有效账号，请检查格式（手机号#密码#AndroidID）\n")
        return
    ...
    for idx, parts in enumerate(accs, 1):
        ...
        if not android_id:
            ...
            summary_blocks.append(f"【账号 {idx}: {mask(phone)}】\n• 账号认证: 缺少AndroidID\n• 任务状态: 任务已跳过")
            continue
