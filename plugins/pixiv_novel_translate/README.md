...
- Google 免费接口是网页版接口，非商用 API，调用过于频繁会被限流（返回 429 + HTML 页面，Loon 侧会显示 `JSON Parse error: Unrecognized token '<'`）。已内置三层防御：识别 429/HTML → 自动进入 60 秒限流窗口（持久化，跨脚本调用生效）→ 窗口内快速失败并提示“限流中”；页面端遇到限流会立即停止整篇翻译，避免加剧限流
- 微软翻译与百度翻译为付费/限额 API，密钥只在 Loon 脚本侧使用，不会下发到页面
- 需要 Loon 保持脚本最新版，插件内已带 `?v=20260828-r3` 版本参数