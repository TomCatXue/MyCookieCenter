...
- `http-request` 匹配 `app-api.pixiv.net/pxtrans`，请求不会发往 Pixiv 上游，直接由 Loon 脚本调用翻译接口并返回 JSON 响应（脚本用 `$done({response:{...}})` 直接返回）
...
- Google 免费接口是网页版接口，非商用 API，调用过于频繁会被限流（返回 429 + HTML 页面，Loon 侧会显示 `JSON Parse error: Unrecognized token '<'`）。已内置多层防御：分块上限加大（350→1500 字符，请求量降约 5 倍）+ 页面本地缓存（重复段落不打 Google）+ 限流识别与 60 秒持久化退避 + 三条域名轮换；窗口内快速失败并停止整篇翻译
- 微软翻译与百度翻译为付费/限额 API，密钥只在 Loon 脚本侧使用，不会下发到页面
- 需要 Loon 保持脚本最新版，插件内已带 `?v=20260828-r4` 版本参数