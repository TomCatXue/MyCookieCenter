...
在 GitHub 官方 App 的星标列表仓库名称后直接显示每个仓库的最近推送时间。

## 功能

- 自动给 `StarredRepositoriesForUser` GraphQL 查询补充 `pushedAt` / `updatedAt`
- 在星标列表每个仓库的名称后追加 `最近推送 · x分钟前 / x小时前 / x天前`
...
- 显示位置为仓库名称后（修改 `name` 字段），不改变列表排序
- 通过 `name` 字段展示，插件会同步剥离点击跳转请求里的展示后缀；若 App 后续改用本地 URL 拼接，需要同步调整
- 状态：待真机验证
