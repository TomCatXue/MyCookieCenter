...
> [`loon/CookieCenter.plugin`](../loon/CookieCenter.plugin) 负责 MITM 凭据捕获与 Loon cron 定时任务；本目录的 BoxJS 订阅提供 Cookie 持久化、参数开关与手动运行入口。
...
| 职责 | 位置 | 说明 |
|---|---|---|
| **MITM + cron** | `loon/CookieCenter.plugin` | 凭据捕获规则 + Loon cron 定时任务，各 App 独立开关 |
| **BoxJS 面板** | `boxjs/CookieCenter.boxjs.json` | Cookie 持久化、偏好设置、任务与手动运行 |
