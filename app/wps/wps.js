 ...
 * WPS · 每日签到 / 福利任务
 *
 * 抓取：打开 WPS App → 个人中心 → 触发 v1mobile 接口 → 抓 Cookie（含 wps_sid/sid）
 * 签到：cron 每日签到 / 福利中心任务
 *
...
function getCookie() {
    const cookie =
        ...
        "";
    if (!cookie || !/(wps_sid|sid)=/.test(cookie)) {
        $.log("未获取到有效 Cookie（缺 wps_sid/sid），跳过");
        return;
    }
    $.setdata(cookie, CK_KEY);
    $.msg($.name, "", "✅ WPS Cookie 获取成功");
}
...
async function main() {
    ...
    // TODO: 替换为真实签到接口与成功判断
    const res = await $.get({
        url: "https://vipapi.wps.cn/checkin/v1",
