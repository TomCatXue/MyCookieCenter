...
if (url.includes('bilibili.app.show.v1.Mixture/RegionList')) {
...
} else if (url.includes('/x/v2/region/cards')) {
    // 【路径 C】本地秒级渲染 4 大卡片分组与顶部快捷访问 HTML 页面
    const htmlHeaders = {
    ...
    // 3. 首页 Tab 与顶栏入口重定向：将右上角分类入口导向内嵌卡片分区页面
    try {
        ...
        if (resJson && resJson.data) {
            ...
            const targetUri = 'bilibili://browser/?url=https%3A%2F%2Fapp.bilibili.com%2Fx%2Fv2%2Fregion%2Fcards&navhide=1';
            let catItem = topMore.find(item => item.id === 'categories' || item.id === 740);
            if (catItem) {
                catItem.uri = targetUri;
            } else {
                topMore.push({
                    id: 'categories',
